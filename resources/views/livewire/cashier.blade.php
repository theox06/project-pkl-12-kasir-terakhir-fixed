                ...
                <div class="col-6 col-lg-4">
                    <div class="card" style="cursor: pointer;" wire:click="addToCart({{ $item->id }})">
                        <div class="card-body p-4">
                            <img src="{{ url('storage/' . $item->image) }}" class="w-75 d-block mx-auto" alt="Dish 01">
                    ...
                    @foreach ($transaction->details as $item)
                        <div class="row align-items-center g-3 mt-3">
                            ...
                            <div class="col-9 col-lg-4">
                                ...
                                <p class="mb-0 fw-semibold text-secondary fs-7">Rp. {{ number_format($item->menu->price) }}</p>
                            </div>
                            <div class="col-4 col-lg-2">
                                <div class="d-flex align-items-center gap-2">
                                    <button class="btn btn-sm btn-quantity rounded-circle" wire:click="quantity_minus({{ $item->id }})">
                                        <i class="bx bx-minus"></i>
                                    </button>
                                    <p class="mb-0 text-dark">
                                        {{ $item->quantity }}
                                    </p>
                                    <button class="btn btn-sm btn-quantity rounded-circle" wire:click="addToCart({{ $item->menu_id }})">
                                        <i class="bx bx-plus"></i>
                                    </button>
                                </div>
                            </div>
                            <div class="col-6 col-lg-3">
                                <p class="mb-0 text-dark fw-bold text-end">Rp. {{ number_format($item->menu->price * $item->quantity) }}</p>
                            </div>
                            <div class="col-2 col-lg-1">
                                <button class="btn btn-sm btn-light btn-delete" type="button" wire:click="delete({{ $item->id }})"><i
